...
| 영역 | 분량 |
|------|------|
| 단어 | **4,678** (`afoqtCommon` 1,164 · P1~P3 테마 대상 2,619) |
| 유추 | **1,130** |
| 독해 | **240지문 / 930문제** |
| 항공 | 문제 **120** · 용어 **93** |
| 수학(산수+대수) | **847** |
...
## 📄 웨이버 증빙 리포트 (v4.150+)
재응시 대기 면제(waiver) 신청용으로, **공식 응시일 이후의 학습을 날짜순으로 증명**하는 자료를 앱이 직접 만든다.

- **기록 방식**: 모든 학습 활동(카드·확인 시험·무한 피드·모의고사·독해 지문·자동 넘김·어근 코치·읽기 화면)이 시작~종료 1건의 이벤트가 된다.
  활성 시간은 *앱이 보이고 최근 60초(읽기 120초·시험 180초) 안에 입력이 있었던 구간*만 센다 — 잠금·백그라운드·유휴 시간은 제외.
- **위조 방지**: 이벤트는 기기별 SHA-256 해시 체인에 추가만 되고 앱은 수정·삭제하지 않는다(초기화해도 로그는 남고 `reset` 이벤트가 기록됨).
  선택 사항으로 Supabase `study_log` 테이블을 만들면 서버 수신 시각(수정 불가)까지 남는다 — `supabase/schema.sql`을 SQL Editor에서 한 번 더 실행하면 된다.
- **리포트**: 홈/통계/설정의 **📄 웨이버 증빙 리포트** → 인쇄(PDF) + `sessions.csv` / `daily.csv` / `exams.csv`. 영문 헤딩·한글 부제, 동기화 코드·개인정보 미포함, 추정 백분위·가상 만점은 제외.
- **기준선**: 설정 → *공식 AFOQT 응시일*(기본 2026-09-18) · *재응시 목표일* · *공식 점수*. 상세 기록 이전 구간은 기존 일별 카운터에서 재구성해 ® 표시로 구분한다.
- 진도 백업(`afoqt-backup-RESTORE-ONLY-*.json`)은 동기화 코드가 들어 있는 **복원용**이라 심사 자료로 제출하면 안 된다.
...
**설정** 또는 **내 약점 리포트 → 📤 LLM용 풀이 기록 내보내기**에서 `.md` 파일을 내려받아 LLM에 첨부하세요. 분석 요청문과 기록을 함께 복사하거나 구조화된 `.json`으로 저장할 수도 있습니다.

- 저장된 시험의 문제·보기·선택 답·정답·해설·풀이 시간, 과목별 누적 통계, 학습/오답 목록의 참고 콘텐츠를 포함합니다.
- 기존 진도와 동기화 데이터를 변경하지 않는 읽기 전용 기능입니다. 동기화 코드와 연결 설정은 내보내지 않습니다. 복원에는 기존 **진도 백업** 파일을 사용하세요.
- 시험 상세는 최근 10회 중 **현재 기기에 남은 기록**만 제공됩니다. 다른 기기 기록은 요약만 동기화되므로 시험을 푼 기기에서 내보내세요. 최대 200회 시험 요약도 포함됩니다.
- 일반 퀴즈 등의 과거 개별 선택 답, 삭제된 상세, 그림·표는 복원할 수 없습니다. 참고 문제집 콘텐츠를 당시의 보기나 선택 답으로 간주하지 않도록 파일에 한계를 표시합니다.

검증: `node scripts/test_study_export.mjs` (원본 불변, 비밀정보 제외, 답 매핑, 누락 문맥, 빈 기록, 오프라인 등록).
